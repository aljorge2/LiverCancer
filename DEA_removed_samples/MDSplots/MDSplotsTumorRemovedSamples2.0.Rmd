...
setwd('~/Desktop/ResearchProjects/LiverCancer/DEA_removed_samples/MDSplots')
```
...
```{r TUmor sample MDS plot}

#creating data frame that lists the sample names, sex, lesion type, and stage 
df <-data.frame(sample= v[["targets"]][["ID"]],sex=v[["targets"]][["sex"]], tumor= v[["targets"]][["tumor"]],stage=v[["targets"]][["Ta"]])

#extracting rownames from expression data and sample ids
row_names_df1 <- colnames(vCorrectLibtype)
column_df2 <- df$sample

# Create a named vector linking row names with samples ids
linkage_vector <- setNames(column_df2, row_names_df1)

# Display the linkage vector
print(linkage_vector)

sample_values_stage_4 <- as.data.frame(df$sample[df$stage == 4])
names_metadata_stage_4 <- as.data.frame(metadata$sampleid[metadata$Ta == 4])
```